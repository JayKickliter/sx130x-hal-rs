...
#[no_mangle]
pub unsafe extern "C" fn lgw_spi_open(
    mut spidev_path: *const libc::c_char,
    mut spi_target_ptr: *mut *mut libc::c_void,
) -> libc::c_int {
...
pub unsafe extern "C" fn lgw_spi_w(
    ...
    out_buf[1] = (0x80i32 | address as libc::c_int >> 8i32 & 0x7fi32) as uint8_t;
    out_buf[2] = (address as libc::c_int >> 0i32 & 0xffi32) as uint8_t;
    out_buf[3] = data;
    command_size = 4i32 as uint8_t;
    /* I/O transaction */
    memset(
...
pub unsafe extern "C" fn lgw_spi_r(
    ...
    let mut spi_device: libc::c_int = 0;
    let mut out_buf: [uint8_t; 5] = [0; 5];
    let mut command_size: uint8_t = 0;
    let mut in_buf: [uint8_t; 5] = [0; 5];
    let mut k: spi_ioc_transfer = spi_ioc_transfer {
        tx_buf: 0,
        rx_buf: 0,
        len: 0,
    ...
    /* prepare frame to be sent */
    out_buf[0] = spi_mux_target;
    out_buf[1] = (0i32 | address as libc::c_int >> 8i32 & 0x7fi32) as uint8_t;
    out_buf[2] = (address as libc::c_int >> 0i32 & 0xffi32) as uint8_t;
    out_buf[3] = 0i32 as uint8_t;
    out_buf[4] = 0i32 as uint8_t;
    command_size = 5i32 as uint8_t;
    /* I/O transaction */
    memset(
        &mut k as *mut spi_ioc_transfer as *mut libc::c_void,
        0i32,
...
pub unsafe extern "C" fn lgw_spi_wb(
    ...
    command[1] = (0x80i32 | address as libc::c_int >> 8i32 & 0x7fi32) as uint8_t;
    command[2] = (address as libc::c_int >> 0i32 & 0xffi32) as uint8_t;
...
pub unsafe extern "C" fn lgw_spi_rb(
    ...
    /* prepare command byte */
    command[0] = spi_mux_target;
    command[1] = (0i32 | address as libc::c_int >> 8i32 & 0x7fi32) as uint8_t;
    command[2] = (address as libc::c_int >> 0i32 & 0xffi32) as uint8_t;
    command[3] = 0i32 as uint8_t;
    command_size = 4i32 as uint8_t;
    size_to_do = size as libc::c_int;
    /* I/O transaction */
    memset(
